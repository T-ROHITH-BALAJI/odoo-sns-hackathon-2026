...
export default function MyTrips() {
    ...
    const currentTrip = {
        name: "Kyoto, Japan",
        dates: "Oct 12 - Oct 24, 2023",
        currentDay: 4,
        totalDays: 12,
        description: "Currently exploring the historic Gion district. The autumn leaves are starting to turn, creating a beautiful contrast with the wooden Machiya houses.",
        nextActivity: "Tea Ceremony @ 3:00 PM",
        weather: { condition: "Sunny", temp: "22°C" },
    ...
    const upcomingTrips = [
        {
            ...
            statusColor: "green",
            travelers: 4,
            image: "https://images.unsplash.com/photo-1498307833015-e7b400441eb8?w=600&h=400&fit=crop"
        },
        {
            id: 2,
            name: "Rome, Italy",
            dates: "Feb 14 - Feb 20, 2024",
            daysUntil: "3 MONTHS",
            progress: 20,
            status: "Just Started",
            statusColor: "yellow",
    ...
    const pastTrips = [
        { name: "Paris, France", date: "June 2023", image: "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?w=400&h=500&fit=crop" },
        { name: "Bali, Indonesia", date: "August 2022", image: "https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=400&h=500&fit=crop" },
        { name: "Krabi, Thailand", date: "Jan 2022", image: "https://images.unsplash.com/photo-1552465011-b4e21bf6e79a?w=400&h=500&fit=crop" },
        { name: "NYC, USA", date: "Dec 2021", image: "https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=400&h=500&fit=crop" }
    ]
                        ...
                        <div className="grid md:grid-cols-2 gap-0">
                            ...
                            <div className="p-8 md:p-10 flex flex-col justify-center">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="font-serif text-3xl font-bold mb-1 text-gray-900 dark:text-white">{currentTrip.name}</h3>
                        ...
                        {upcomingTrips.map((trip) => (
                            <article key={trip.id} className="bg-white dark:bg-zinc-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 group border border-gray-100 dark:border-zinc-700 flex flex-col">
                                ...
                                <div className="p-6 flex flex-col flex-grow">
                                    <h3 className="font-serif text-xl font-bold mb-1 text-gray-900 dark:text-white">{trip.name}</h3>
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{trip.dates}</p>
                                    <div className="mt-auto">
                                        <div className="flex -space-x-2 mb-4">
                                            {[...Array(Math.min(trip.travelers, 3))].map((_, i) => (
                                                <div key={i} className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-800 bg-gray-200 dark:bg-zinc-600"></div>
                                            ))}
                                            {trip.travelers > 3 && (
                                                <div className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-800 bg-gray-100 dark:bg-zinc-700 flex items-center justify-center text-xs font-bold text-gray-500">+{trip.travelers - 3}</div>
                                            )}
                                        </div>
                                        <div className="w-full bg-gray-100 dark:bg-zinc-700 rounded-full h-2 mb-2">
                                            <div className={`h-2 rounded-full ${trip.statusColor === 'green' ? 'bg-green-500' : 'bg-yellow-400'}`} style={{ width: `${trip.progress}%` }}></div>
                                        </div>
                                        <div className="flex justify-between text-xs font-medium text-gray-500 dark:text-gray-400">
                                            <span>Planning Status</span>
                                            <span className={trip.statusColor === 'green' ? 'text-green-500' : 'text-yellow-500'}>{trip.status}</span>
                                        </div>
                                    </div>
                                </div>
