...
import {
    ...
    DollarSign, Users, Clock, Trash2, Edit2, Save, Hotel,
...
export default function BuildItinerary() {
    ...
    // Check if this is a demo trip (no database)
    const isDemo = tripId?.startsWith('demo-')

    useEffect(() => {
        if (tripId) {
            if (isDemo) {
                // Demo mode - no database calls, just set loading false
                setLoading(false)
            } else {
                fetchTrip()
                fetchStops()
    ...
    const fetchStops = async () => {
        ...
        try {
            ...
            // Add UI-only fields to each stop
            const stopsWithUIFields = (data || []).map(stop => ({
                ...stop,
            ...
            setStops(stopsWithUIFields)
            if (stopsWithUIFields.length > 0) {
                setExpandedStop(stopsWithUIFields[0].id)
            }
        } catch (error) {
    ...
    const updateTrip = async () => {
        if (isDemo) return // Demo mode - no database updates
        setSaving(true)
        try {
