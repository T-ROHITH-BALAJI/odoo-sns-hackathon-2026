...
export default function AdminDashboard() {
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const recentTrips = trips?.filter(t => new Date(t.created_at) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))?.length || 0
            const totalCities = cities?.length || 0
            const totalActivities = activities?.length || 0

            // Trip timeline breakdown (past vs upcoming)
            const now = new Date()
            ...
            const avgBudget = totalTrips > 0 ? totalExpenses / totalTrips : 0
            ...
            setStats({
                ...
                popularDestinations,
                categoryData,
                engagementRate: userCount > 0 ? ((totalTrips / userCount)).toFixed(2) : 0
            })
        } catch (error) {
